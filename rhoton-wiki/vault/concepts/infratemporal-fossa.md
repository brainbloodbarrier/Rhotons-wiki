---
title: Infratemporal Fossa
category: concepts
tags:
  - anatomy
  - skull-base
  - cranial-base
  - rhoton
aliases:
  - ITF
  - "fossa infratemporalis"
sources:
  - "Rhoton - Cranial Anatomy and Surgical Approaches (2023), Ch.6, 14"
created: 2026-04-09
updated: 2026-04-09
summary: >-
  V3 territory, maxillary artery, pterygoid plexus
parent: "[[cranial-base]]"
---

# Infratemporal Fossa

The infratemporal fossa is an irregularly shaped deep space below the middle cranial fossa, posterior to the maxilla, and medial to the ramus of the mandible. It is a critical corridor in lateral skull base approaches and contains the pterygoid muscles, mandibular nerve (V3), maxillary artery, and pterygoid venous plexus.

## Boundaries

- **Superior** — greater wing of the [[sphenoid-bone]] (infratemporal surface) and floor of the middle cranial fossa
- **Medial** — lateral pterygoid plate ([[sphenoid-bone]]); communicates with the pterygopalatine fossa through the pterygomaxillary fissure
- **Lateral** — ramus of the mandible and zygomatic arch
- **Anterior** — posterior surface of the maxilla
- **Posterior** — opens into the area around the carotid sheath, parapharyngeal space, and [[foramen-magnum]] region

## Contents

### Neural
- **[[trigeminal-nerve|V3]] (mandibular nerve)** — exits the foramen ovale ([[sphenoid-bone]]) into the roof of the fossa; divides into anterior (motor: temporalis, masseter, pterygoids) and posterior (sensory: auriculotemporal, inferior alveolar, lingual) trunks
- **Chorda tympani** — joins the lingual nerve (taste and parasympathetic to submandibular gland)
- **Otic ganglion** — parasympathetic relay below the foramen ovale

### Vascular
- **Maxillary artery** — the larger terminal branch of the external carotid; courses through the fossa giving off multiple branches (middle meningeal, inferior alveolar, deep temporal, buccal, sphenopalatine)
- **Middle meningeal artery** — ascends through the foramen spinosum ([[sphenoid-bone]]) to supply the dura of the middle fossa
- **Pterygoid venous plexus** — extensive venous network around the pterygoid muscles; communicates with the [[cavernous-sinus]] via emissary veins through the foramen ovale and foramen of Vesalius

### Muscular
- **Lateral pterygoid** — two heads; inserts on the mandibular condyle and TMJ disc
- **Medial pterygoid** — deep to the lateral pterygoid; inserts on the medial mandibular ramus
- **Temporalis** (partial) — passes through the fossa to insert on the coronoid process

## Communications

| Connection | Via | Leads to |
|---|---|---|
| Middle cranial fossa | Foramen ovale, foramen spinosum | [[sphenoid-bone]], [[cranial-base]] |
| Pterygopalatine fossa | Pterygomaxillary fissure | Nasal cavity, [[orbit]], palate |
| [[orbit]] | Inferior orbital fissure | Orbital floor |
| Pharynx/parapharyngeal space | Posterior opening | [[internal-carotid-artery]], [[foramen-magnum]] |
| [[cavernous-sinus]] | Emissary veins (foramen ovale) | Pterygoid venous plexus |

## Surgical Significance

### Preauricular infratemporal approach
Provides lateral access to the middle and posterior cranial fossae, [[cavernous-sinus]], petrous [[internal-carotid-artery|ICA]], and jugular foramen. The zygomatic arch is removed and the mandibular condyle reflected to expose the fossa contents.

### Anterolateral approach to foramen magnum
The infratemporal fossa opens posteriorly into the area around the carotid sheath and the [[vertebral-artery]] (ascending between C2 and C1 transverse processes). Removing fossa contents exposes the vertebral artery V3 segment laterally.

### Middle fossa tumors
Tumors of the infratemporal fossa (schwannomas, meningiomas, juvenile nasopharyngeal angiofibromas) are accessed through the preauricular infratemporal or transzygomatic approaches.

### Percutaneous trigeminal procedures
Percutaneous needle for trigeminal balloon compression or radiofrequency enters through the cheek, traverses the infratemporal fossa, and reaches the foramen ovale → [[meckels-cave]].

## Relations

- Below the middle cranial fossa ([[cranial-base]], [[sphenoid-bone]])
- Contains [[trigeminal-nerve|V3]] (mandibular nerve) exiting foramen ovale
- Maxillary artery courses through — middle meningeal artery to dura
- Pterygoid venous plexus communicates with [[cavernous-sinus]]
- Opens posteriorly toward [[internal-carotid-artery]] and [[vertebral-artery]]
- Communicates with pterygopalatine fossa (route to nasal cavity and [[orbit]])
- Related to [[temporal-bone]] (mandibular fossa, petrous carotid)
- See [[surgical-approaches-overview]] for lateral skull base approach planning
- Percutaneous access to [[meckels-cave]] via foramen ovale

## References

- Rhoton, Ch.6 — The Anterior and Middle Cranial Base (infratemporal fossa anatomy)
- Rhoton, Ch.14 — The Foramen Magnum (anterolateral approaches through the fossa)
